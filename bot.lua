	...
	if data.ID == "UpdateNewMessage" then
		...
		if msg.content_.ID == "MessageText" then
			...
			if is_naji(msg) then
				...
				elseif text:match("^(وضعیت)$") or text:match("^(2)$") or text:match("^(atash2012)$") then
					...
					local txtadd = redis:get("botBOT-IDaddmsgtext") or  "شماره تون ذخیره شد✍"
...
📠 شماره های  دخیره شده : 
🆗➡️🚥  <b>]] .. tostring(contacts)..[[</b><code> contacts </code>
⛓💾 لینک های ذخیره شده : 
...
🆔Ⓜ️ @atash2012]]
				...
				elseif text:match("^(ترک کردن) (.*)$") then
					...
					return rem(matches)
				elseif text:match("^(افزودن به همه) (%d+), (%d+), (%d+), (%d+)$") then
					local matches = text:match("%d+"), ("%d+"), ("%d+"), ("%d+")
